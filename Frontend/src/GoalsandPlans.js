...
import { X, Target, Plus, Search, BookOpen, Play, FileText, MessageSquare, ClipboardList, BarChart3, Youtube, FileImage } from 'lucide-react';
...
const CreateAssetModal = ({ isOpen, onClose, onCreateAsset }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!loggedInUserId) {
      setSubmitError('Please log in to create assets');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {!loggedInUserId && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-700 text-sm font-medium">Please log in to create assets</p>
            </div>
          )}
...
const CreateProgramModal = ({ isOpen, onClose, onCreateProgram, assets }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!loggedInUserId) {
      setSubmitError('Please log in to create programs');
      return;
    }
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {!loggedInUserId && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-700 text-sm font-medium">Please log in to create programs</p>
            </div>
          )}
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button
              ...
              disabled={isSubmitting || !loggedInUserId || programData.selectedAssets.length === 0}
            >
              {isSubmitting ? (
                <>
                  <div className="animate-spin mr-2 w-4 h-4 border-2 border-white border-t-transparent rounded-full"></div>
...
const GoalsAndPlans = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!loggedInUserId) {
        setLoading(false);
        setError('Please log in to view your assets and programs');
        return;
      }
      ...
      <div className="min-h-screen bg-gray-50 pt-16 px-6 pb-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-600 mb-2">Please log in to access Goals & Plans</h3>
            <p className="text-gray-500">You need to be logged in to view and manage your assets and programs.</p>
          </div>
        </div>
      </div>
    );
                ...
                <div className="text-center py-12">
                  ...
                  {!searchTerm && (
                    <button
                      ...
                      disabled={assets.length === 0 || !loggedInUserId}
                    >
                      <Plus size={20} className="mr-2" />
